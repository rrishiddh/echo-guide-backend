...
export const updateBookingStatusSchema = z.object({
  status: z
    ...
    .refine((val) => Object.values(BookingStatus).includes(val), {
...
export const bookingQuerySchema = z.object({
  touristId: z.string().regex(/^[0-9a-fA-F]{24}$/, "Invalid tourist ID format").optional(),
  guideId: z.string().regex(/^[0-9a-fA-F]{24}$/, "Invalid guide ID format").optional(),
  listingId: z.string().regex(/^[0-9a-fA-F]{24}$/, "Invalid listing ID format").optional(),
  ...
  sortBy: z
    .string()
    .regex(/^-?(bookingDate|createdAt|totalPrice|status)$/, "Invalid sort field")
    .optional(),
});
